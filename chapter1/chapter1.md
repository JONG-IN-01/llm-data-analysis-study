...
| 검증 항목 | 확인 내용 |
| --- | --- |
| 선택한 LLM 제안 | 성별 선호 상품 카테고리(Top 5) 파악 |
| 필요한 파일 | customer, order_items, product |
| 필요한 컬럼 | gender, customer_id, order_status, order_id, product_id, category  |
| 계산 범위 | LLM의 제안에서는 completed와 canceled를 어떻게 나눌 지 여부가 포함되지 않았다. 또한, 성별 선호시 판매금액으로 할지 판매 갯수로 할지 구체적으로 정해지지 않았다.|
...
아직 실제 데이터로 성별 선호 상품 카테고리(Top 5)를 검증하지 못하였고, completed된 상품만 포함할 것인지 canceled된 상품까지 포함할 것인지 결정하지 못하였다.
