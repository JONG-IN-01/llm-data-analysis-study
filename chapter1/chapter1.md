...
예를 들어, 특정 제품의 광고를 더 추가로 한다던지 특정 제품의 매출이 감소하는 이유를 파악하여 소비자들이 어떤 제품을 선호하는지 파악할 수 있다.
...
질문 : 성별에 따른 카테고리별 completed 주문금액 비교하고 싶다이기 때문에 성별과 카테고리별로 completed한 주문금액과
성별과 카테고리 completed한 상태를 나타내주는 열이 있는지 파악이 필요했고 해당 열들을 연결해줄 수 있는 기본키, 외래키 등이 필요했다.
...
![STEP 3 LLM Prompt와 응답](images/step03_llm_response1.png)
![STEP 3 LLM Prompt와 응답](images/step03_llm_response2.png)
---
...
| 검증 항목 | 확인 내용 |
| --- | --- |
| 선택한 LLM 제안 | 성별 선호 상품 카테고리(Top 5) 파악 / 남성과 여성이 각각 완료한 주문 내역을 바탕으로, 가장 많이 팔린(수량 기준 또는 금액 기준) 상품 카테고리를 추출하여 성별 타겟팅 상품군을 확인합니다. |
| 필요한 파일 | customer, order_items, product, orders|
| 필요한 컬럼 | gender, customer_id, order_status, order_id, product_id, category  |
| 계산 범위 | LLM의 제안에서는 성별 선호시 판매금액으로 할지 판매 갯수로 할지 구체적으로 정해지지 않았다. 분석가인 내가 금액으로 할지 수량으로 할 지 의사결정한다.|
| 실제 데이터 확인 필요 여부 | 실제 데이터 확인이 필요하다. |
| 원인 단정 여부 | 원인을 단정하고 있지 않다. |
| 최종 판단 | 수정 후 사용 |

### 내가 수정한 내용

```text
성별 선호 상품 카테고리(Top 5) 파악에서 선호한다는 정의를 더욱 구체적으로 판매횟수로 정할 것인지 판매금액으로 정할 것이지 고려해볼것이다.
```

### 결과 관찰

LLM이 데이터 분석의 기초를 제공할 때는 실제 데이터를 제공하지 않으면 내가 가지고 있는 데이터에 없는 내용이 필요할 수 있고, 만약 인과관계나 상관관계가 있는 문제라면 LLM이 이러한 문제를 단정하고 있는지 파악할 필요가 있다. 현재 성별 선호 상품 카테고리(Top 5) 파악 질문은 단순비교이기 때문에 문제를 단정하고 있지 않다고 생각한다. 

### 나의 해석과 판단

수정 후 사용이라고 판단을 내린 이유는 금액이나 구매 횟수에 따라 성별 선호 상품 카테고리를 충분히 구할 수 있을 것이라고 생각했기 때문이며, 성별별 구매 패턴을 분석할 때 가장 많이 사는 카테고리들을 구분해놓는 것도 중요하다고 생각했기 때문이다. 

### 업무·분석적 의미

LLM 제안을 검증하지 않고 바로 사용하는 경우 자신이 가지고 있는 데이터에 없는 칼럼이 필요할 수 있을 것이고, 구체적으로 어떤 기준으로 데이터를 분석해야 하는지를 놓칠 수 있기에 분석하려는 사람이 LLM이 제안해준 분석을 구체적으로 생각을 해보며 더 필요한 조건이 있는지 파악할 필요가 있다.

### 한계와 추가 확인 사항

아직 실제 데이터로 성별 선호 상품 카테고리(Top 5)를 검증하지 못하였다.
...
- 사용 목적: 주어진 데이터에서 분석하고 싶은 내용을 어떻게 분석하면 좋을 지 방법을 물어보기 위한 목적.
- 입력 Prompt 요약: 목적은 성별에 따른 completed 주문 기준 금액과 구매 패턴을 이해하기 위해 데이터 분석 초보자가 수행할 분석질문 5개 추천
- LLM 답변 요약:
- 1. 성별 기준 총매출 및 객단가(ARPU/AOV) 차이 확인
2. 성별에 따른 구매 빈도 및 재구매 패턴 분석
3. 성별 선호 상품 카테고리(Top 5) 파악
4. 성별 1회 주문 시 장바구니 크기(Basket Size) 비교
5. 성별에 따른 구매 시점(요일/시간대) 트렌드 확인
- 실제 반영 여부: 3번을 구체적으로 살펴보았는데, 수정을 해서 사용하기로 했다.
- 사람이 검증한 항목: LLM이 제공한 제안이 실제 데이터 내에서 실현 가능한지 여부를 검증했다.
- 사람이 수정한 내용: LLM이 제공한 제안에서 구체적인 의사결정이 필요한 부분을 수정했다.
- 남은 확인 사항: 실제로 분석을 시작하지는 않았다.

### 결과 관찰

LLM 사용 기록을 정리하면서 LLM이 제공한 답변에서 내가 구체화하거나 결정할 수 있는 부분을 결정할 수 있게 된다.

### 나의 해석과 판단
왜 필요한지 자신의 말로 작성하세요.
Prompt Log를 남김으로서 LLM이 답변한 내용을 실제 데이터에서 적용할 수 있는지 파악할 수 있는 능력을 기를 수 있고, Prompt Log를 진행하면 할수록 LLM에 질문하는 Skill이 성장할 것이라고 생각한다. 또한, LLM은 전체적인 과정을 먼저 제공해주기에 구체적인 의사결정은 사람이 해야한다는 것을 알 수 있었다.
...
이번 실습에서 실제이름, raw데이터를 올리면 안된다고 생각했습니다.
또, 내부 URL이 캡처에 보이지 않도록 하였습니다.
...
데이터분석을 하기 전에 라이브러리를 불러오고 기본 환경을 세팅하는 것이다.

#### 한계와 추가 확인 사항

customers = pd.read_csv(DATA_DIR / 'customers.csv')
customers.head() 이 코드는 오류가 났다. 
파일을 연결하지 않아서 그런 거 같다.
chapter2에서 보충해야할 거 같다.
...
```text
이번 장에서는 LLM에 질문을 모호한 질문이 아닌 최대한 구체적으로 해야한다는 것을 알게 되었고, LLM이 제공해주는 대답을 맹목적으로 따르지 않고 LLM을 이용하여 다루는 사람이 LLM의 답변을 검증하는 것이 중요한 과정이겠구나 생각했다.
```

### LLM을 데이터 분석에 사용할 때 가장 조심해야 할 점

```text
외부로 유출되어서는 안되는 데이터는 함부로 LLM에 올리면 안된다고 생각했다. 아직까지는 그런 데이터를 다룬 적이 없지만, 이어지는 실습에서는 조심해야할 것 같다고 생각했다.
```
...
| --- | --- | --- |
| 질문 정의 | 데이터 분석 목적을 제공하면 큰 틀을 제공할 수 있다. | 데이터를 두고 무엇을 분석할 것인지 의사결정해야한다. |
| 데이터 확인 | 만약 사람이 데이터를 소스로 제공하면 더 큰 도움을 줄 수 있을 것이다.  | LLM이 제공한 답변이 실제 데이터에 적용 가능한지 판단한다. |
| 코드 작성 | 코딩을 잘하지 못하는 사람에게 도움을 줄 수 있다. | 코드를 작성하고 실행하는 과정에서 개인정보 유출이든지, 내부자료 유출에 대한 책임이 있다. |
| 결과 해석 | 전체적인 부분에서 도와줄 수 있다.  | LLM이 해석한 부분이 실제 데이터에 부합하는지 체크를 할 필요가 있다. |
| 최종 판단 | 사람이 최종판단을 하는데 의견 및 근거를 제공해줄 수 있다.  | 최종판단에 대한 책임은 사람에게 있다. |

### 다음 Chapter에서 확인하고 싶은 것

```text
import셀 다음에 제공해준 customers 코드는 오류가 났는데 왜 오류가 났는지 궁금하다.
```
...
```text
LLM 질문 중 하나를 선택해서 LLM이 제공한 2개 선택지 중 하나를 선택하는 과정을 거쳤고, LLM이 제공해준 질문이 raw data에서 적용 가능한지 살펴보았다.
```

### 아직 확인이 필요한 내용 1개

```text
GIT허브 사용이 미숙하여 제대로 된 지 모르겠으며, 코드에서 import 준비까지는 완료했는데 밑에 제공해준 코드가 왜 오류난 지 모르겠다.
로컬 GIT은 사용할 줄 모르기에 사용하는 방법을 배우고 싶다.
```
